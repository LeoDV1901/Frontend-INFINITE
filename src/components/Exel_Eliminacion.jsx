...
const CriteriosEliminacionForm = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://api.weareinfinite.mx/form/criterios_eliminacion');
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Criterios Eliminación');

    // Descargar el archivo Excel
    XLSX.writeFile(wb, 'criterios_eliminacion_datos.xlsx');
  };

  return (
    <div className="container">
      <h2>Formulario de Criterios de Eliminación</h2>
...
export default CriteriosEliminacionForm;
